...
<body>
	<%
		...
		if (eksikAlanlar.trim().length() > 0) {
			request.setAttribute("eksikAlanlar", eksikAlanlar); //Yukarıda eksik alanları topladı 0 dan büyükse sayfada bıraktı
			
			%>
			<jsp:forward page="02_BilgiFormu.jsp"></jsp:forward> <!--Sayfayı geri yönlendiriyor, diyor ki sayfanı düzelt öyle gel BilgiFormunuKontrolEt.jsp'ye -->
			<%
		}
	%>
	...
	<table border="5" id="bilgiTablosuSonDurum">
		...
		<tr>
			<td width="30%">Adı</td>
			<td><input type="text" name="adi" size="20"
				value="<%=request.getParameter("adi")%>"></td>
		</tr>

		<tr>
			<td width="30%">Parola</td>
			<td><input type="text" name="parola" size="20"
				value="<%=request.getParameter("parola")%>"></td>
		</tr>

		<tr>
			<td width="30%">Adres</td>
			<td><input type="text" name="adres" size="20"
				value="<%=request.getParameter("adres")%>"></td>
		</tr>

		<tr>
			<td width="30%">Telefon</td>
			<td><input type="text" name="telefon" size="20"
				value="<%=request.getParameter("telefon")%>"></td>
		</tr>

		<tr>
			<td width="30%">Ülke</td>
			<td><input type="text" name="ulke" size="20"
				value="<%=request.getParameter("ulke")%>"></td>
		</tr>

		<tr>
			<td width="30%">Şehir</td>
			<td><input type="text" name="sehir" size="20"
				value="<%=request.getParameter("sehir")%>"></td>
		</tr>
